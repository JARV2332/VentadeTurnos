-- RPC de búsqueda con org explícita (solo servidor / service_role).
-- Evita get_user_organizacion_id y permite 1 sola consulta desde Vercel.

CREATE INDEX IF NOT EXISTS idx_usuarios_app_auth_user
  ON public.usuarios_app(auth_user_id)
  WHERE activo = true;

CREATE OR REPLACE FUNCTION public.buscar_boleta_entrega_org(
  p_codigo TEXT,
...
BEGIN
  ...
  WHERE t.id IN (
    SELECT DISTINCT NULLIF(elem->>'turno_id', '')::uuid
    FROM jsonb_array_elements(v_brazos) AS elem
    WHERE NULLIF(elem->>'turno_id', '') IS NOT NULL
  );

  v_turno_id := NULLIF(v_brazos->0->>'turno_id', '')::uuid;
  v_turno := CASE WHEN v_turno_id IS NOT NULL THEN v_turnos_map->v_turno_id::text ELSE NULL END;
  ...
  WHERE cg.id = NULLIF(v_brazos->0->>'cargador_id', '')::uuid
  LIMIT 1;
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'brazo', bj,
        'turno', v_turnos_map->(bj->>'turno_id')
      )
      ORDER BY NULLIF(bj->>'numero_turno', '')::int NULLS LAST, bj->>'numero_brazo', bj->>'lado'
    ),
    '[]'::jsonb
  )
  INTO v_items
...
REVOKE ALL ON FUNCTION public.buscar_boleta_entrega_org(TEXT, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.buscar_boleta_entrega_org(TEXT, UUID) TO service_role;
